window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + STYLE_TAG + '"]') === null) {
      ...
      tag.textContent = [
        '.dsh-kb-root { position: fixed; left: 50%; bottom: 22px; transform: translateX(-50%); z-index: 40; pointer-events: none; display: flex; align-items: flex-end; gap: 16px; opacity: 0.75; }',
        '.dsh-kb { display: flex; flex-direction: column; gap: 5px; }',
        '.dsh-kb-row { display: flex; gap: 5px; }',
        '.dsh-kb-key { height: 30px; display: flex; align-items: center; justify-content: center; border-radius: 6px; border: 1px solid rgba(140,140,140,0.5); background: rgba(32,32,34,0.88); color: #e8e8e8; font-size: 10px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; box-shadow: 0 2px 0 rgba(0,0,0,0.5); transition: transform 50ms ease, box-shadow 50ms ease, background 120ms ease, color 120ms ease; }',
        '.dsh-kb-key.on { transform: translateY(2px); box-shadow: 0 0 0 rgba(0,0,0,0.5); background: rgba(88,150,255,0.42); color: #fff; border-color: rgba(120,180,255,0.85); }',
        '.dsh-mouse { position: relative; width: 58px; height: 90px; border-radius: 28px 28px 22px 22px; border: 1px solid rgba(140,140,140,0.5); background: rgba(32,32,34,0.88); box-shadow: 0 2px 6px rgba(0,0,0,0.5); }',
        '.dsh-mouse-btn { position: absolute; top: 0; width: 50%; height: 40px; border-bottom: 1px solid rgba(140,140,140,0.4); transition: background 80ms ease; }',
        '.dsh-mouse-btn.left { left: 0; border-radius: 28px 0 0 0; }',
        '.dsh-mouse-btn.right { right: 0; border-radius: 0 28px 0 0; }',
        '.dsh-mouse-btn.on { background: rgba(88,150,255,0.42); }',
        '.dsh-mouse-wheel { position: absolute; top: 20px; left: 50%; transform: translateX(-50%); width: 9px; height: 20px; border-radius: 5px; border: 1px solid rgba(140,140,140,0.5); background: rgba(95,95,100,0.9); transition: background 80ms ease, top 80ms ease; }',
        '.dsh-mouse-wheel.on { background: rgba(88,150,255,0.7); }',
    ...
    // 87-key TKL layout: [code, label, width-unit]
    var ROWS = [
      [
      ...
      [
        ['ControlLeft','Ctrl',1.5], ['MetaLeft','Cmd',1.3], ['AltLeft','Alt',1.3], ['Space','',6],
        ['AltRight','Alt',1.3], ['MetaRight','Cmd',1.3], ['ControlRight','Ctrl',1.5]
      ]
    ]

    var ARROWS = [['ArrowUp','Up'], ['ArrowLeft','Lt'], ['ArrowDown','Dn'], ['ArrowRight','Rt']]
    ...
    function KeyboardView(props) {
      ...
      rows.push(React.createElement('div', { className: 'dsh-kb-row', key: 'arrows' },
        ARROWS.map(function (k) {
          return React.createElement(Key, { key: k[0], label: k[1], w: 1, on: !!props.pressed[k[0]] })
        }),
      ))
      return React.createElement('div', { className: 'dsh-kb' }, rows)
    }
    ...
    function Overlay() {
      ...
      React.useEffect(function () {
        ...
        function btnName(b) { return b === 0 ? 'left' : (b === 2 ? 'right' : (b === 1 ? 'middle' : null)) }
        function mouseDown(e) {
          var key = btnName(e.button)
          if (!key) return
          setMouse(function (prev) {
            if (prev[key]) return prev
            var n = { left: prev.left, right: prev.right, middle: prev.middle, wheel: prev.wheel }
            n[key] = true
            return n
          })
        }
        ...
        var wheelTimer = null
        function onWheel() {
          setMouse({ left: mouse.left, right: mouse.right, middle: mouse.middle, wheel: true })
      ...
      return React.createElement('div', { className: 'dsh-kb-root' },
        React.createElement(KeyboardView, { pressed: pressed }),

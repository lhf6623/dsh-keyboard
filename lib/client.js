window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + STYLE_TAG + '"]') === null) {
      ...
      tag.textContent = [
        '.dsh-kb-root { position: fixed; left: 50%; bottom: 22px; transform: translateX(-50%); z-index: 40; pointer-events: none; display: flex; align-items: flex-end; gap: 16px; opacity: 0.62; }',
    ...
    function KeyboardView(props) {
      ...
      // 方向键：倒 T 布局（上单独一行，左/下/右一行）
      rows.push(React.createElement('div', { key: 'arrows', style: { display: 'flex', flexDirection: 'column', gap: '5px' } },

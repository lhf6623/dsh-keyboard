window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + STYLE_TAG + '"]') === null) {
      ...
      tag.textContent = [
        '.dsh-kb-root { position: fixed; left: 50%; bottom: 22px; transform: translateX(-50%); z-index: 40; pointer-events: none; opacity: 0.5; }',
    ...
    function Overlay() {
      ...
      React.useEffect(function () {
        function measure() {
          ...
          if (frame) {
            var cols = getComputedStyle(frame).gridTemplateColumns.split(' ').map(function (v) { return parseFloat(v) || 0 })
            var sidebarW = cols[0] || 0
            var detailsW = cols[2] || 0

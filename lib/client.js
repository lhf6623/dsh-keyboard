  ...
  factory: function (require) {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + STYLE_TAG + '"]') === null) {
      ...
      tag.textContent = [
        ...
        '.dsh-kb-wrap { display: flex; align-items: flex-end; gap: 10px; }',
        ...
        '.dsh-mouse { position: fixed; top: 20px; right: 20px; z-index: 40; pointer-events: none; width: 58px; height: 90px; border-radius: 28px 28px 22px 22px; border: 1px solid rgba(0,0,0,0.4); background: rgba(255,255,255,0.5); box-shadow: 0 1px 4px rgba(0,0,0,0.2); }',
    ...
    function KeyboardView(props) {
      var mainRows = ROWS.map(function (row) {
      ...
      var mainBlock = React.createElement('div', { className: 'dsh-kb' }, mainRows)
      // 方向键：右下角倒 T 布局
      var arrowBlock = React.createElement('div', { className: 'dsh-kb-arrows' },
        React.createElement('div', { className: 'dsh-kb-row' },
          React.createElement('div', { style: { width: '30px' } }),
          React.createElement(Key, { label: 'Up', w: 1, on: !!props.pressed['ArrowUp'] }),
          React.createElement('div', { style: { width: '30px' } }),
        ),
        React.createElement('div', { className: 'dsh-kb-row' },
          React.createElement(Key, { label: 'Lt', w: 1, on: !!props.pressed['ArrowLeft'] }),
          React.createElement(Key, { label: 'Dn', w: 1, on: !!props.pressed['ArrowDown'] }),
          React.createElement(Key, { label: 'Rt', w: 1, on: !!props.pressed['ArrowRight'] }),
        ),
      )
      return React.createElement('div', { className: 'dsh-kb-wrap' },
        mainBlock,
        arrowBlock,
      )
    }
    ...
    function Overlay() {
      ...
      return React.createElement('div', null,
        React.createElement('div', { className: 'dsh-kb-root' },

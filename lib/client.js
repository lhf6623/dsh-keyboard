window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var KB_CONFIG_KEY = 'dsh-vibe.config'
    var KB_CONFIG_LEGACY_KEY = 'dsh-keyboard.config'
    ...
    function normalizeKbConfig(c) {
      var shake = c && (c.shake === 'light' || c.shake === 'medium') ? c.shake : 'off'
      return {
        enabled: !(c && c.enabled === false),
        flame: !(c && c.flame === false),
        shake: shake,
        sound: !(c && c.sound === false),
    ...
    function loadKbConfig() {
      try {
        if (typeof window !== 'undefined' && window.localStorage) {
          ...
          if (!raw) raw = window.localStorage.getItem(KB_CONFIG_LEGACY_KEY)
          if (raw) {
            var cfg = normalizeKbConfig(JSON.parse(raw))
            try { window.localStorage.setItem(KB_CONFIG_KEY, JSON.stringify(cfg)) } catch (e) {}
            return cfg
          }
        }
      } catch (e) {}
      return { enabled: true, flame: true, shake: 'off', sound: true, opacity: 0.5, scale: 1 }
    }
    ...
    function triggerShake(card) {
      var level = kbConfig.shake
      if (level === 'off' || reducedMotion()) return
      if (!card || typeof card.animate !== 'function') return
      var amp = level === 'medium' ? 2 : 1
      var dur = level === 'medium' ? 180 : 120
      var keyframes = [
      ...
      shakeAnim = card.animate(keyframes, { duration: dur, easing: 'ease-out' })
    }
    ...
    function playAnswerSound() {
      if (kbConfig.sound === false) return
      var ctx = null
      try { ctx = ensureAudio() } catch (e) {}
      if (!ctx) return
      try {
    ...
    function shakePage() {
      ...
      var amp = level === 'medium' ? 3 : 2
      var dur = level === 'medium' ? 220 : 160
      var keyframes = [
        { transform: 'translateX(0)' },
        { transform: 'translateX(' + amp + 'px)' },
        { transform: 'translateX(' + (-amp) + 'px)' },
      ...
      answerShakeAnim = target.animate(keyframes, { duration: dur, easing: 'ease-out' })
    }

    function spawnFlame(x, y) {
      if (reducedMotion() || kbConfig.flame === false) return
      if (!flameCtx) return
      var n = 16
      for (var i = 0; i < n; i++) {
    ...
    function Overlay() {
      ...
      React.useEffect(function () {
        ...
        function onMouse(e) { primeAudio(); applyButtons(e.buttons || 0) }
    ...
    function VibeSection() {
      ...
      return React.createElement('div', { className: 'dsh-kb-settings' },
        React.createElement('div', { className: 'dsh-kb-group' },
          React.createElement('div', { className: 'dsh-kb-group-title' }, '键盘外观'),
          React.createElement('div', { className: 'dsh-kb-settings-row' },
            React.createElement('label', null, '显示键盘'),
            React.createElement('input', { type: 'checkbox', checked: cfg.enabled, onChange: function (e) { update({ enabled: e.target.checked }) } }),
          ),
          React.createElement('div', { className: 'dsh-kb-settings-row' },
            React.createElement('label', null, '键盘透明度'),
          ...
          React.createElement('div', { className: 'dsh-kb-settings-row' },
            React.createElement('label', null, '键盘缩放'),
        ...
        React.createElement('div', { className: 'dsh-kb-group' },
          React.createElement('div', { className: 'dsh-kb-group-title' }, '打字反馈'),
          React.createElement('div', { className: 'dsh-kb-settings-row' },
            React.createElement('label', null, '打字火焰'),
            React.createElement('input', { type: 'checkbox', checked: cfg.flame, onChange: function (e) { update({ flame: e.target.checked }) } }),
          ),
          React.createElement('div', { className: 'dsh-kb-settings-row' },
            React.createElement('label', null, '输入抖动'),
            React.createElement('div', { className: 'dsh-kb-seg' },
              React.createElement('button', { type: 'button', className: 'dsh-kb-seg-btn' + (cfg.shake === 'off' ? ' on' : ''), onClick: function () { update({ shake: 'off' }) } }, '关'),
              React.createElement('button', { type: 'button', className: 'dsh-kb-seg-btn' + (cfg.shake === 'light' ? ' on' : ''), onClick: function () { update({ shake: 'light' }) } }, '轻'),
              React.createElement('button', { type: 'button', className: 'dsh-kb-seg-btn' + (cfg.shake === 'medium' ? ' on' : ''), onClick: function () { update({ shake: 'medium' }) } }, '中'),
            ),
          ),
        ),
        React.createElement('div', { className: 'dsh-kb-group' },
          React.createElement('div', { className: 'dsh-kb-group-title' }, '回答反馈'),
          React.createElement('div', { className: 'dsh-kb-settings-row' },
            React.createElement('label', null, '回答提示音'),
            React.createElement('input', { type: 'checkbox', checked: cfg.sound, onChange: function (e) { update({ sound: e.target.checked }) } }),
          ),
        ),
      )
    }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject('settings.section', function () {
        return ctx.slots.register(
          { name: 'settings.section', id: 'vibe', order: 5, label: function () { return '氛围' } },
          VibeSection,
        )
      })
      if (typeof EventSource !== 'undefined') {
        ctx.effect(function () {
          var es = new EventSource('/api/vibe-events')

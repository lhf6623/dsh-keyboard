...
import { onMouseDown, onMouseUp, onMouseMove, onMouseLeave, onWheel } from '../lib/events/mouse'
...
export function useMouseState(): MouseState {
  ...
  React.useEffect(() => {
    ...
    function onMouse(e: MouseEvent) { applyButtons(e.buttons || 0) }
    ...
    return () => {
      offDown(); offUp(); offMove(); offLeave(); offWheel(); offPage()
      if (wheelTimer) window.clearTimeout(wheelTimer)
    }
  }, [])

...
const ROW = [
  'vibe-flex', 'vibe-items-center', 'vibe-gap-2',
  'vibe-py-4',
  'vibe-border-0', 'vibe-border-b', 'vibe-border-solid', 'vibe-border-[var(--dsw-alias-border-l2)]',
].join(' ')
const ROW_LAST = ROW + ' vibe-border-b-0'
// 主开关标题行：加粗标题 + 左右对称，无底边框（关闭时不留残线；非首组加顶部边框分隔）
const HEAD_ROW = 'vibe-flex vibe-items-center vibe-justify-between vibe-gap-2 vibe-py-4'
const HEAD_ROW_DIV = HEAD_ROW + ' vibe-border-0 vibe-border-t vibe-border-solid vibe-border-[var(--dsw-alias-border-l2)] vibe-mt-3'
// 子项行：左缩进形成层级
const INDENT = ' vibe-pl-6'
const ROW_TEXT = 'vibe-flex vibe-flex-col vibe-flex-1 vibe-gap-1 vibe-min-w-0'
const TITLE = 'vibe-text-[14px] vibe-leading-[22px] vibe-text-[var(--dsw-alias-label-primary)]'
const DESC = 'vibe-text-[12px] vibe-leading-[18px] vibe-text-[var(--dsw-alias-label-tertiary)]'
// 主开关行标题：加粗，与子项区分
...
const VALUE = 'vibe-text-[12px] vibe-text-[var(--dsw-alias-label-tertiary)] vibe-tabular-nums vibe-min-w-10 vibe-text-right'
const PILL = [
  'vibe-box-border', 'vibe-border', 'vibe-border-solid',
  'vibe-rounded-[16px]', 'vibe-px-[14px]', 'vibe-py-[6px]', 'vibe-text-[13px]', 'vibe-cursor-pointer',
  'vibe-transition-[background-color,border-color]',
].join(' ')
const PILL_ON = 'vibe-bg-[var(--dsw-alias-bg-module-platform)] vibe-border-[var(--dsw-static-neutral-bluish-400)] vibe-text-[var(--dsw-alias-label-primary)]'
const PILL_OFF = 'vibe-bg-transparent vibe-border-[var(--dsw-alias-border-l2)] vibe-text-[var(--dsw-alias-label-secondary)]'

function Row(props: { title: string; desc?: string; last?: boolean; indent?: boolean; children: React.ReactNode }) {
  return (
    <div className={(props.last ? ROW_LAST : ROW) + (props.indent ? INDENT : '')}>
  ...
  return (
    <div className="vibe-flex vibe-flex-col">
      ...
      <div className={HEAD_ROW_DIV}>
        ...
        <input className={CHECKBOX} type="checkbox" checked={cfg.feedback} onChange={(e) => update({ feedback: e.target.checked })} />
      </div>
      {cfg.feedback && (
        <>
            ...
            <div className="vibe-inline-flex vibe-gap-1.5">
              {SHAKE_LEVELS.map((level) => (
                <button key={level} type="button"
                  className={PILL + ' ' + (cfg.shake === level ? PILL_ON : PILL_OFF)}
      ...
      <div className={HEAD_ROW_DIV}>
        ...
        <input className={CHECKBOX} type="checkbox" checked={cfg.response} onChange={(e) => update({ response: e.target.checked })} />
      </div>
      {cfg.response && (
        <>
          ...
          <Row title="整页抖动强度" indent>
            <div className="vibe-inline-flex vibe-gap-1.5">
              {SHAKE_LEVELS.map((level) => (
                <button key={level} type="button"
                  className={PILL + ' ' + (cfg.pageShakeLevel === level ? PILL_ON : PILL_OFF)}
